 ...
 *   5. Drop last freq bin (2049 to 2048)
 *   6. No frame trimming — htdemucs_embedded model expects all frames
...
async function loadFromCache() {
  try {
    ...
    return new Promise((resolve) => {
      ...
      const req = tx.objectStore('models').get(MODEL_KEY);
...
async function saveToCache(buffer) {
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      tx.objectStore('models').put(buffer, MODEL_KEY);
...
export async function loadDemucsModel(progressCallback) {
  progressCallback?.({ stage: 'checking', percent: 0 });
